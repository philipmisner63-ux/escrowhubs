...
const NETWORK_STATE_LABELS: Record<NetworkState, { label: string; color: string; dot: string }> = {
  ...
  correct_network: { label: "Polygon (Chain 137)",             color: "text-green-400",  dot: "bg-green-400"  },
};
              ...
              <div className="space-y-4">
                ...
                <div className={cn(
                  ...
                  {networkState === "wrong_network" && (
                    <p className="text-red-300">⚠️ Your wallet is on the wrong network. Switch to <strong>Polygon (Chain ID 137)</strong> to use this app.</p>
                  )}
                  {networkState === "switching" && (
                    <p className="text-yellow-300">🔄 Switching to Base… approve in your wallet.</p>
                  )}
                  {networkState === "correct_network" && (
                    <p className="text-green-300">✅ Connected to Polygon Mainnet (Chain ID 137)</p>
                  )}
                </div>
              </div>
            )}
