...
import { polygonMainnet, getRpcUrl, DEFAULT_CHAIN_ID } from "@/lib/chains";
...
const rpcClient = createPublicClient({ chain: polygonMainnet, transport: http(getRpcUrl(DEFAULT_CHAIN_ID)) });
...
function MilestoneEscrowView({ address }: { address: Address }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Evidence panel — shown when any milestone is disputed and arbiter is AI */}
      {data.milestones.some(m => m.state === MilestoneState.DISPUTED) &&
        arbiterAddress &&
        data.arbiter?.toLowerCase() === arbiterAddress.toLowerCase() && (
        <EvidencePanel escrowAddress={address} />
      )}
...
const ACCEPTED_FILE_TYPES = "image/*,.pdf,.txt,.md,.json,.csv,.doc,.docx";
...
function EvidencePanel({ escrowAddress }: { escrowAddress: Address }) {
  ...
  const chainId                           = useChainId();
  const arbiterAddress                    = getArbiterAddress(chainId);
  const ipfsConfigured                    = !!process.env.NEXT_PUBLIC_IPFS_ENABLED;
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (f.size > MAX_FILE_SIZE_MB * 1024 * 1024) {
      addToast({ type: "error", message: `File too large — max ${MAX_FILE_SIZE_MB}MB` });
      return;
    }
    setFile(f);
    setUploadedUri(null); // reset if new file chosen
  }
  ...
  async function uploadToIPFS(): Promise<string | null> {
    if (!file && !evidenceText.trim()) return null;
    setUploading(true);
    const pid = addToast({ type: "pending", message: "Uploading evidence to IPFS…" });
    try {
      const form = new FormData();
      if (file) form.append("file", file, file.name);
      form.append("text", evidenceText.trim());
      form.append("escrowAddress", escrowAddress);
      form.append("submitter", wallet ?? "");

      const res  = await fetch("/api/upload-evidence", { method: "POST", body: form });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ?? "Upload failed");

      removeToast(pid);
      addToast({ type: "success", message: "Evidence uploaded to IPFS" });
      setUploadedUri(data.uri);
      return data.uri as string;
    } catch (e: unknown) {
      removeToast(pid);
      addToast({ type: "error", message: e instanceof Error ? e.message.slice(0, 120) : "Upload failed" });
      return null;
    } finally {
      setUploading(false);
    }
  ...
  async function submitEvidence() {
    if (!evidenceText.trim() && !file) return;
    setSubmitting(true);

    try {
      // Determine what URI to submit on-chain
      let uri: string;
      if (ipfsConfigured && (file || evidenceText.trim())) {
        // Upload to IPFS and submit the IPFS URI
        const uploaded = await uploadToIPFS();
        if (!uploaded) { setSubmitting(false); return; }
        uri = uploaded;
      } else {
        // No IPFS — submit plain text directly
        uri = evidenceText.trim();
      }

      const pid = addToast({ type: "pending", message: "Submitting evidence on-chain…" });
  ...
  const canSubmit = !submitting && !uploading && (!!evidenceText.trim() || !!file);
  ...
  return (
    <GlassCard className="p-5">
      ...
      <p className="text-xs text-slate-400 mb-3">AI Arbitration Engine — Patent Pending</p>
      <p className="text-xs text-slate-400 mb-4">
        The AI oracle is monitoring this dispute. Submit your written statement and optionally attach
        a screenshot, document, or other file. All evidence is stored permanently on IPFS and on-chain.
      </p>

      {/* Text statement */}
      <textarea
        className="w-full rounded-xl bg-white/5 border border-violet-400/20 px-4 py-3 text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-violet-400/50 focus:ring-1 focus:ring-violet-400/20 transition-colors resize-none mb-3"
        rows={4}
        placeholder="Describe your case clearly and factually… e.g. 'Work was not delivered by the agreed deadline. The attached screenshot shows the conversation confirming the due date.'"
        value={evidenceText}
      ...
      <div className="mb-4">
        {!file ? (
          <button
            ...
            className="flex items-center gap-2 text-xs text-slate-400 border border-dashed border-white/10 rounded-xl px-4 py-3 w-full hover:border-violet-400/40 hover:text-violet-300 transition-colors"
          >
            <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
            </svg>
            Attach a file (screenshot, PDF, document — max {MAX_FILE_SIZE_MB}MB)
          </button>
        ) : (
          <div className="flex items-center gap-3 border border-violet-400/20 rounded-xl px-4 py-3 bg-violet-400/5">
            ...
            {uploadedUri && (
              <span className="text-xs text-emerald-400 shrink-0">✓ On IPFS</span>
            )}
            <button
              type="button"
              onClick={removeFile}
