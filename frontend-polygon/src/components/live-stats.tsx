  ...
  return (
    <div ref={ref} className="card-glow-stats rounded-xl border border-white/8 bg-white/3 p-5 relative">
      ...
      {loading ? (
        ...
        >
          {prefix}{display}{suffix !== " ETH" && suffix !== "%" ? suffix : ""}
          {suffix === " ETH" && !loading && (
            <span className="text-xl ml-1 text-cyan-400/70">ETH</span>
          )}
          {suffix === "%" && null}
        </p>
      )}
...
export function LiveStats() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      const client = createPublicClient({ chain: polygonMainnet, transport: http(getRpcUrl(DEFAULT_CHAIN_ID)) });
