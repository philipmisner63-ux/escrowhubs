...
import { useAccount, WagmiContext, useSignMessage } from "wagmi";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const message = `EscrowHubs API Auth\nAction: notifications-read\nWallet: ${wallet}\nTimestamp: ${timestamp}`;
        const signature = await signMessageAsync({ message });
        const r = await fetch(`/api/notifications/preferences?wallet=${wallet}&timestamp=${timestamp}&signature=${encodeURIComponent(signature)}`);
