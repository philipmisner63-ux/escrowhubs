...
export function EscrowCard({ escrow }: EscrowCardProps) {
  ...
  return (
    <Link href={`/escrow/${escrow.id}`}>
      <div className="glass rounded-xl border border-white/8 p-5 hover:border-cyan-400/30 hover:shadow-[0_0_30px_rgba(0,245,255,0.06)] transition-all duration-300 cursor-pointer group">
        {/* Header */}
        <div className="flex items-start justify-between gap-3">
          ...
          <span className={cn(
            "text-xs font-medium px-2.5 py-1 rounded-full border shrink-0 capitalize",
        ...
        <div className="mt-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm font-semibold text-cyan-400">{escrow.amount}</p>
          </div>
